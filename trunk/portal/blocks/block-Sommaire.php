...
///// on récupère la table des groupes si on gère les groupes ////
...
$flagmenu = 0;  // flag qui est mis automatiquement à "1" quand il y a un module dans la rubrique 99
	...
	$totalcategorymodules[$totalcompteur]=$row2['module']; //utile quand groupmenu=99 -->cette variable liste tous les modules affichés dans des catégories
	$compteur2=$categorie;
					...
					if (!($module[$z]==$main_module && $moduleinthisgroup[$som_groupmenu][$keyinthisgroup]!="Lien externe")) { //on n'affiche pas le module en homepage, sauf s'il est appelé par un lien externe
			    		if (($is_admin===1 AND $view[$z] == 2) OR $view[$z] != 2) { //si on n'est pas admin et que le module est réservé aux admins, il n'apparaît pas
							if ($nomdumodule==$module[$z]) {
								if ($dynamic==1 && $detectMozilla!=1 && ereg(addcslashes($urldumodule, '?&'), $_SERVER['REQUEST_URI'])) { // si la page visualisée est le module[$z], alors on récupère son groupmenu pour ne pas enrouler la catégorie par défaut.
								...
								elseif ($is_user==0 && $view[$z]==1 && ($invisible[0]==2 || $invisible[0]==4)) {//visiteur non membre, ne peut pas visualiser un module réservé aux membres.
									$limage="<img src=\"$path_icon/admin/interdit.gif\" title=\""._SOMRESTRICTED."\">";
								}
								
								if ($is_user==1 && $view[$z]==1 && $invisible[0]==5 && $isin==0 && $is_admin==0) { //c'est un membre, mais pas dans le bon groupe pour voir le module.
								}
								elseif($is_user==0 && $view[$z]==1 && $invisible[0]==5 && $is_admin==0) { //c'est un visiteur, il doit être membre et faire partie d'un groupe pour voir le module.
								}
								elseif ($is_user==0 AND $view[$z]==1 AND $invisible[0]==3 && $is_admin==0) { //c'est un visiteur, il doit être membre pour voir le module (pas de gestion des groupes)
								}
								else {
