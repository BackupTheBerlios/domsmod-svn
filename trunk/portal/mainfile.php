...
function adminblock() {
	...
	// START: HACK - ADVANCED USER MANAGER
		...
		$npending = $db->sql_numrows($db->sql_query("SELECT * FROM ".$prefix."_users_temp"));
		if ($npending)
		{
			$admin_file="admin";
			$content .= "<strong><big>&middot;</big></strong>&nbsp;<img src='images/activate.gif' border='0' align='absmiddle' />&nbsp;<a href='".$admin_file.".php?op=aumInactiveRegList'>"._AUM_PENDINGUSERS."</a>: $npending<br></span>";
		};
		...
		if ($n_dupconfig)
		{
			...
			if ($allowSchedule)
			{
				...
				global $user_prefix;
				$n_deadusers = $db->sql_numrows($db->sql_query("SELECT * FROM ".$user_prefix."_users WHERE user_lastvisit < '$duptime' AND user_id <> 1"));
				if ($n_deadusers)
				{
					$content .= "<strong><big>&middot;</big></strong>&nbsp;<img src='images/deaduser.gif' border='0' align='absmiddle' />&nbsp;<a href='".$admin_file.".php?op=aumDeadUserList'>"._AUM_DEADUSERS."</a>: $n_deadusers<br></span>";
				};
			};
		};

		// prune waiting queue entries
		$n_pqentries = $db->sql_numrows($db->sql_query("SELECT * FROM ".$prefix."_aum_pruneq"));
		if ($n_pqentries)
		{
			$content .= "<strong><big>&middot;</big></strong>&nbsp;<img src='images/pqentry.gif' border='0' align='absmiddle' />&nbsp;<a href='".$admin_file.".php?op=aumPruneQList'>"._AUM_PQENTRIES."</a>: $n_pqentries<br></span>";
		};
